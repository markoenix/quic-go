...
func dial(
	...
	c, err := newClient(conn, connIDGenerator, config, tlsConf, use0RTT)
	if err != nil {
		return nil, err
	}
...
func newClient(sendConn sendConn, connIDGenerator ConnectionIDGenerator, config *Config, tlsConf *tls.Config, use0RTT bool) (*client, error) {
	if tlsConf == nil {
		tlsConf = &tls.Config{}
	} else {
...
func (c *client) dial(ctx context.Context) error {
	...
	go func() { errorChan <- c.conn.run() /* returns as soon as the connection is closed */ }()
